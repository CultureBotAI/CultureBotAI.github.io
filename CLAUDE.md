...
- `_layouts/default.html`, `_includes/head.html`, `_includes/header.html`, `_includes/footer.html` - Minima overrides
- `assets/custom.css` - site styles, including the fleet page colour tokens
...
Two layers, refreshed differently:
- **Derived:** the census, overlaps, stats, stat strip and capability table,
  regenerated by the pipeline over snapshots of each Mech pinned at its GitHub
  `main`. The census, the subsets, the stats and `_fleet/data/site_audit.json`
...
- **MediaIngredientMech** (`/mediaingredientmech/`) - LLM-assisted curation of media-ingredient mappings; its records map mostly to ChEBI, with MeSH, CAS, NCIT, MicrO and FOODON for the rest, and none to PubChem or METPO (#150).
