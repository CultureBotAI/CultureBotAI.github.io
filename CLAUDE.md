...
- The organization profile README lives in the separate `CultureBotAI/.github`
  ...
  mentions only CultureMech, with a stale "10,000+ recipes" figure (#213).
...
- **Fleet membership and capabilities:** the membership badges, the capability
  table and the stat strip's Mech count come from a snapshot of
  culturebotai-claw's fleet manifest, `_fleet/data/manifest.json`, refreshed with
...
- **Derived:** the census, overlaps, per-card stat lines, and the stat strip's
  vocabulary and merged-PR tiles, regenerated by the pipeline: record counts and
  vocabularies over snapshots of each Mech pinned at its GitHub `main`, merged-PR
  counts from GitHub search when `mech_stats.py` runs. The census, the subsets,
